...
export default function TypeField({ data, tid, fid }: TypeFieldProps) {
  ...
  const [editField, setEditField] = useState<Partial<ITypeField>>({});
              ...
              {dbToTypes[database][data.type]?.isSized && (
                <>
                  ...
                  <InputNumber
                    ...
                    placeholder={t("size")}
                    value={data.size as number}
                    readOnly={layout.readOnly}
                    onChange={(value) =>
                      updateType(tid, {
                        fields: types[tid].fields.map((e, id) =>
                          id === fid ? { ...data, size: value as number } : e,
                        ),
                      })
                    }
                    onFocus={(e: any) => setEditField({ size: e.target.value })}
                    onBlur={(e: any) => {
              ...
              {dbToTypes[database][data.type]?.hasPrecision && (
                <>
                  ...
                  <Input
                    ...
                    readOnly={layout.readOnly}
                    validateStatus={
                      /^\(\d+,\s*\d+\)$|^$/.test(data.size as string)
                        ? "default"
                        : "error"
                    }
