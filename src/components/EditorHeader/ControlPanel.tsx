...
export default function ControlPanel({
  ...
  const hasSelectedRelationships = useMemo(() => {
    return bulkSelectedElements.some((e) => e.type === ObjectType.RELATIONSHIP);
  }, [bulkSelectedElements]);

  const rotateRelationshipName = () => {
    ...
    selectedRels.forEach((relSelection) => {
      const rel = relationships.find((r) => r.id === relSelection.id);
      if (rel) {
        ...
        ...prev,
        {
          ...
          message: t("edit_relationship", {
            refName: selectedRels.length === 1 
              ? relationships.find(r => r.id === selectedRels[0].id)?.name 
