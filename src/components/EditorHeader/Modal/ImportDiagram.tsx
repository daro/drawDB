...
import { DB, IMPORT_FROM, STATUS, tableWidth } from "../../../data/constants";
import {
  useAreas,
  useEnums,
  useNotes,
...
export default function ImportDiagram({
  ...
  const loadJsonData = (file, e) => {
    ...
    let ok = true;
    jsonObject.relationships = (jsonObject.relationships || []).map((rel) => {
      const newRel = { ...rel };
      if (newRel.divider && !(newRel.waypoints || []).some(wp => wp.mode === "divider")) {
        newRel.waypoints = [
          ...(newRel.waypoints || []),
          {
            ...newRel.divider,
    ...
    jsonObject.tables.forEach((table) => {
      table.width = table.width ?? tableWidth;
  ...
  const loadDBMLData = async (e) => {
    try {
      const result = await fromDBML(e.target.result, tables, enums);
