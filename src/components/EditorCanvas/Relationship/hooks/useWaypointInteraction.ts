...
import { ObjectType, Action, GRID_CONFIG } from "@data/constants";
...
export function useWaypointInteraction(
  ...
  useEffect(() => {
    ...
    const handlePointerMove = (e: PointerEvent) => {
      ...
      let targetX = x - dragging.grabOffset.x;
      let targetY = y - dragging.grabOffset.y;

      if (settings.snapToGrid) {
        targetX = Math.round(targetX / GRID_CONFIG.SIZE) * GRID_CONFIG.SIZE;
        targetY = Math.round(targetY / GRID_CONFIG.SIZE) * GRID_CONFIG.SIZE;
      }
  ...
  }, [dragging, pointer.spaces.diagram, data.waypoints, data.id, data.name, updateRelationship, setUndoStack, setRedoStack, t, selectedElement.initialWaypoints, settings.snapToGrid]);
