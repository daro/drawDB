...
export function useCanvasInteraction(
  ...
  const handlePointerDownOnElement = useCallback((e: React.PointerEvent | PointerEvent, { id, type }: { id: string | number, type: number }) => {
    ...
    switch (type) {
      ...
      case ObjectType.AREA:
        element = areas.find((a) => a.id === id);
        break;
      case ObjectType.NOTE:
        element = notes.find((n) => n.id === id);
        break;
      ...
      case ObjectType.XOR_GROUP:
        element = xorGroups.find((g) => g.id === id);
        break;
      case ObjectType.OR_GROUP:
        element = orGroups.find((g) => g.id === id);
        break;
      default:
        break;
    }
    ...
    console.log(`[Interaction] handlePointerDownOnElement type: ${type}, id: ${id}`);
    console.log(`[Interaction] Current bulkSelectedElements before logic:`, bulkSelectedElements);
    ...
    const isInBulkSelected = bulkSelectedElements.some((el) =>
    ...
    if (e.ctrlKey || e.metaKey) {
      ...
      if (isInBulkSelected) {
        setBulkSelectedElements((prev) =>
          prev.filter((el) => !isSameElement(el, elementInBulk))
        );
        if (selectedElement.id === element.id && selectedElement.element === type) {
          setSelectedElement((prev: any) => ({
            ...prev,
  ...
  }, [selectedElement, layout.sidebar, setSelectedElement, setBulkSelectedElements, isSameElement, pointer.spaces.diagram, tables, areas, notes, texts, relationships, xorGroups, orGroups]);
  ...
  const handlePointerMoveDragging = useCallback(() => {
    if (isDragging()) {
      ...
      const moveDeltaX = mainElementFinalX - currentCoords.x;
      const moveDeltaY = mainElementFinalY - currentCoords.y;

      if (moveDeltaX === 0 && moveDeltaY === 0) return true;

      const newBulkSelectedElements: IBulkSelectedElement[] = [];
      bulkSelectedElements.forEach((el) => {
        let finalX = (el.currentCoords?.x ?? 0) + moveDeltaX;
        let finalY = (el.currentCoords?.y ?? 0) + moveDeltaY;
        ...
        if (el.type === ObjectType.AREA) {
          updateArea(el.id, { x: finalX, y: finalY });
        }
        if (el.type === ObjectType.NOTE) {
          updateNote(el.id, { x: finalX, y: finalY });
        }
        if (el.type === ObjectType.TEXT) {
          updateText(el.id, { x: finalX, y: finalY });
        }
        newBulkSelectedElements.push({
          ...el,
          currentCoords: { x: finalX, y: finalY },
