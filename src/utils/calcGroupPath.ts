...
export function calcGroupPath(isRight, x, width, height, radius = 0, offset = 0) {
  ...
  const segments: PathArray = [["M", startX, 0]];

  if (radius <= 0) {
    segments.push(["L", targetX, 0]);
    segments.push(["L", targetX, height]);
    segments.push(["L", startX, height]);
    return SVGPathCommander.pathToString(segments);
  }

  const r = Math.min(radius, Math.abs(width), height / 2);
  const sweepOuter = isRight ? 1 : 0;
  
  segments.push(["L", targetX - r * direction, 0]);
  segments.push(["A", r, r, 0, 0, sweepOuter, targetX, r]);
  segments.push(["L", targetX, height - r]);
  segments.push(["A", r, r, 0, 0, sweepOuter, targetX - r * direction, height]);
  segments.push(["L", startX, height]);
